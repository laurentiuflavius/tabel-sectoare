...
- **Persoane**: nume, zi de naștere (persoana e liberă în acea zi), perioadă de indisponibilitate (ex. plecată de la o dată), număr maxim de zile de sector pe lună.
...
2. Încarcă fișierele proiectului. Cel mai simplu, din pagina repository-ului: **Add file → Upload files**, apoi trage toate fișierele și folderul `lib/` (cu `index.html`, `style.css`, `app.js`, `lib/docx.umd.js`, `README.md`) și apasă **Commit changes**.
   - Alternativ, din linia de comandă:
     ```bash
     git init
     git add index.html style.css app.js lib/docx.umd.js README.md
...
lib/docx.umd.js   biblioteca docx (inclusă local, fără dependențe externe)
```
...
- Documentul generat este A4 vedere (landscape), cu titlul „…– Luna <Luna> <Anul>", în formatul tabelelor lunare existente.
